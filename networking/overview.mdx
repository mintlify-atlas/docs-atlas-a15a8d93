...
description: "SilverOS includes a minimal network stack built on an RTL8139 Ethernet driver, supporting ARP resolution, IPv4 packet I/O, and ICMP ping requests and replies."
---

SilverOS ships a purpose-built network stack layered directly over bare-metal x86\_64 hardware. Starting from PCI enumeration and RTL8139 NIC initialisation, the stack climbs through Ethernet II framing, ARP address resolution, and IPv4 routing, up to ICMP echo request and reply handling. There is no operating-system abstraction between your code and the wire — every received frame is dispatched in the RTL8139 IRQ handler and every outbound byte is written directly to an I/O-mapped transmit buffer.
...
When the RTL8139 signals a receive interrupt, the IRQ handler reads the raw Ethernet frame out of the ring buffer and passes it — pointer and byte length — to:

...
1. **Automatically on login** — after `user_login()` succeeds, `desktop_init` calls `terminal_open()` immediately so the terminal is ready the moment the desktop appears.
...
| `reboot` | `reboot` | Reboots via keyboard controller (`0x64 = 0xFE`) — only in shell mode |
